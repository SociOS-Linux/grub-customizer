	...
	public Gc::Controller::Helper::RuleMoverConnection
{
	...
	private: Gc::Common::Exception::CmdExecException thrownException; //to be used from the die() function
	...
	public: void init()
	{
		if ( !grublistCfg
			...
			throw Gc::Common::Exception::ConfigException("init(): missing some objects", __FILE__, __LINE__);
		}
	...
	public: void loadThreadedAction(bool preserveConfig)
	{
		...
		try {
			if (!is_loading){ //allow only one load thread at the same time!
				...
				} catch (Gc::Common::Exception::FileReadException e) {
				...
				} catch (Gc::Common::Exception::CmdExecException const& e){
	...
	public: void setViewOptionAction(Gc::Common::Type::ViewOption option, bool value)
	{
		...
		try {
			...
			} catch (Gc::Common::Exception::FileSaveException e) {

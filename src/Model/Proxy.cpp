...
struct Model_Proxy : public Proxy {
	...
	bool generateFile(std::string const& path, int cfg_dir_prefix_length, std::string const& cfg_dir_noprefix, std::map<Model_Entry const*, Model_Script const*> entrySourceMap, std::map<Model_Script const*, std::string> const& scriptTargetMap) {
		if (this->dataSource){
 ...
 //before running this function, the realted script file must be saved!

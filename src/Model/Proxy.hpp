...
class Model_Proxy : public Proxy
{
	...
	{
		auto& ruleList = this->getRuleList(this->getParentRule(rule));
	
		auto el = this->getListIterator(rule, ruleList);
		std::shared_ptr<Model_Rule> parent = nullptr;
		try {
			parent = this->getParentRule(rule);
		} catch (ItemNotFoundException const& e) {} // leave parent in nullptr state
		auto& sourceList = this->getRuleList(parent);
	
	
		auto next = this->rules.end();
		...
		auto targetList = &this->getRuleList(parent);
	
		std::shared_ptr<Model_Rule> newRule = std::make_shared<Model_Rule>(*rule);
	
		auto insertPos = next;
	
		if (needToGoUp) {
			auto parentSubmenu = this->getParentRule(rule);
			targetList = &this->getRuleList(this->getParentRule(parentSubmenu));
			insertPos = this->getListIterator(parentSubmenu, *targetList);
			if (direction == 1) {
				insertPos++;
			}
		} else if (next->get()->type == Model_Rule::SUBMENU) {
			targetList = &next->get()->subRules;
		...
		if (ruleList.size() == 1) {
			...
			auto parent = this->getParentRule(rule);
			auto& parentOfParent = this->getRuleList(this->getParentRule(this->getParentRule(rule)));
	...
	public: void merge(std::shared_ptr<Model_Proxy> foreignProxy, int direction) {
		if (direction == -1) {
			for (auto iter = foreignProxy->rules.begin(); iter != foreignProxy->rules.end(); iter++) {
			...
			for (auto iter = foreignProxy->rules.rbegin(); iter != foreignProxy->rules.rend(); iter++) {
				if (iter->get()->isVisible) {
					this->removeEquivalentRules(*iter);

...
LiveCDSetupDialog::LiveCDSetupDialog(bool isLiveCD)
	...
	lblSubmountpointDescription(gettext("These are the mountpoints of your fstab file.\nPlease select every grub/boot related partition."), Gtk::ALIGN_LEFT)
{
	...
	chkCustomPartition.signal_toggled().connect(sigc::mem_fun(this, &LiveCDSetupDialog::signal_custom_partition_toggled));
	txtCustomPartition.signal_changed().connect(sigc::mem_fun(this, &LiveCDSetupDialog::signal_custom_partition_typing));
	bttMountFs.signal_clicked().connect(sigc::mem_fun(this, &LiveCDSetupDialog::signal_btt_mount_click));
	bttUmountFs.signal_clicked().connect(sigc::mem_fun(this, &LiveCDSetupDialog::signal_btt_umount_click));
	
	lvRootPartitionSelection.get_selection()->signal_changed().connect(sigc::mem_fun(this, &LiveCDSetupDialog::signal_lvRootPartitionSelection_changed));
...
void LiveCDSetupDialog::readPartitionInfo(){
...
void LiveCDSetupDialog::signal_custom_partition_toggled(){
	updateSensitivity();
}

void LiveCDSetupDialog::signal_lvRootPartitionSelection_changed(){
	updateSensitivity();
}

void LiveCDSetupDialog::signal_custom_partition_typing(){
	updateSensitivity();
}

void LiveCDSetupDialog::generateSubmountpointSelection(FILE* fstabFile){
	...
	for (MountTable::iterator iter = fstab.begin(); iter != fstab.end(); iter++){
		if (iter->mountpoint[0] == '/' && iter->mountpoint.length() > 1 && iter->mountpoint != "/proc"){
			...
			cb->signal_toggled().connect(sigc::bind<Gtk::CheckButton&>(sigc::mem_fun(this, &LiveCDSetupDialog::submountpoint_toggle), *cb));
...
void LiveCDSetupDialog::signal_btt_mount_click(){
...
void LiveCDSetupDialog::signal_btt_umount_click(){
...
void LiveCDSetupDialog::updateSensitivity(){
...
void LiveCDSetupDialog::on_cancel(){
	this->is_cancelled = true;
	this->hide();
}

void LiveCDSetupDialog::on_apply(){
	this->hide();
}

void LiveCDSetupDialog::submountpoint_toggle(Gtk::CheckButton& sender){
...
std::string LiveCDSetupDialog::getRootMountpoint() const {
	return mountpoint;
}

bool LiveCDSetupDialog::isCancelled() const {
	return is_cancelled;
}
